<?php
/*
    Моудль новостей (стандартные HTML-страницы с датой публикации)
*/
    class classNews extends classModule{
        ...
        public function getItem($id=false, $lng){
            ...
            if ($id){
                $sql = $Db->query("
                    SELECT `items`.`date` AS `items_date`, `items`.`public` AS `items_public`, `items`.`home` AS `items_home`, `descr`.`lng` AS `descr_lng`, `descr`.`title` AS `descr_title`, `descr`.`descr` AS `descr_descr`, `descr`.`keywords` AS `descr_keywords`, `descr`.`content` AS `descr_content`
        ...
        public function editItem($id=false, $item){
            ...
            if ($item['photo']) $Img->addImage($id, 'news', 'photo');
            return $id;
        }
        public function getItems($lng, $w=false, $l=true){
            ...
            $sql   = $Db->prepare("
                SELECT `items`.`id` AS `items_id`, `items`.`date` AS `items_date`, `items`.`public` AS `items_public`, `items`.`home` AS `items_home`, `descr`.`title` AS `descr_title`, `descr`.`descr` AS `descr_descr`, `descr`.`keywords` AS `descr_keywords`, `descr`.`content` AS `descr_content`
                FROM `mod_{$this->module}_items` AS `items`
                    LEFT JOIN `mod_{$this->module}_descr` AS `descr` ON `descr`.`id` = `items`.`id` AND `descr`.`lng` = '$lng'
                $where $like $dateFrom $dateTo $w
                $order
            ...
            while ($row = $sql->fetch()){
                ...
                $row['items_photo'] = $Img->getImages($row['items_id'], 'news');
                $items[]            = $row;
            }
            return $items;
        }
