<?php
    class classAjax{
        ...
        public function pagesChangePublic($arr){
            global $Db;
            extract($arr, EXTR_OVERWRITE);
            $sql    = $Db->query("SELECT `public` FROM `mod_pages_items` WHERE `id` = '$id'");
            $row    = $sql->fetch();
            $public = $row['public'] == 1 ? 0 : 1;
            $Db->query("UPDATE `mod_pages_items` SET `public` = '$public' WHERE `id` = '$id'");
            return $public;
        }
        public function pagesDelItem($arr){
            global $Db;
            extract($arr, EXTR_OVERWRITE);
            $Db->query("DELETE FROM `mod_pages_items` WHERE `id` = '$id'");
            $Db->query("DELETE FROM `mod_pages_descr` WHERE `id` = '$id'");
            return true;
        }
